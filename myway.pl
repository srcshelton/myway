...
sub main( @ ) { # {{{
	...
	if( ( defined( $help ) and $help ) or ( 0 == scalar( @ARGV ) ) ) {
		...
		if( defined( $odbcdsn ) or ( $syntax eq 'vertica' ) ) {
			...
			print( ( " " x $length ) . "[--notice] [--warn] [--debug]\n" );
			print( "\n" );
			print( ( " " x $length ) . "backup options:   [--compress [:scheme:]] [--transaction]\n" );
			...
			print( ( " " x $length ) . "--transaction    - Don't lock transactional tables\n" );
			...
			print( ( " " x $length ) . "--separate-files - Create backup file per-database or per-table\n" );
			print( ( " " x $length ) . "--skip-metadata  - Don't include myway metadata tables in backup\n" );
			print( ( " " x $length ) . "--skip-reorder   - Don't sort data by primary key before writing\n" );
			print( ( " " x $length ) . "--ddl            - Only backup database structure\n" );
			print( ( " " x $length ) . "--dml            - Only backup database data\n" );
			print( "\n" );
			print( ( " " x $length ) . "  * Backup defaults:\n" );
			print( "\n" );
			print( ( " " x $length ) . "--skip-definer   - Avoid writing node-locking DEFINER statements\n" );
			print( ( " " x $length ) . "--skip-drop      - Avoid writing DROP statements\n" );
			print( ( " " x $length ) . "--skip-procedures- Avoid writing Procedure definitions\n" );
			print( ( " " x $length ) . "--extended-insert- Write multiple rows at once\n" );
			print( "\n" );
